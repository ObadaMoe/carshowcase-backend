...
public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<PaymentDetails>(e =>
        {
            e.ToTable("PaymentDetails");
            e.Property(p => p.CardHolder).IsRequired().HasMaxLength(100);
            e.Property(p => p.CardNumber).IsRequired().HasMaxLength(16);
            e.Property(p => p.ExpDate).IsRequired().HasMaxLength(5);
            e.Property(p => p.CVV).IsRequired().HasMaxLength(3);
        });
    }
